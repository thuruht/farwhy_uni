...
// ----------------------------------------------------------------------
// Session cleanup utility for Farewell/Howdy Unified Project
// This script clears expired session tokens from the KV store
// ----------------------------------------------------------------------
...
rl.question('Are you sure you want to proceed? (yes/no): ', (answer) => {
  ...
  // Execute wrangler command to list KV keys with expired sessions
  exec('npx wrangler kv:key list --binding=FWHY_KV', (error, stdout, stderr) => {
    ...
    try {
      ...
      // Filter for session keys (which start with 'session:')
      ...
      // Examine expiration time for each session key
      const now = Date.now();
      ...
      rl.question(`Delete ${expiredKeys.length} expired session keys? (yes/no): `, (confirm) => {
        ...
        const deleteNextKey = (index) => {
          if (index >= expiredKeys.length) {
            console.log(`Session cleanup completed. Deleted ${deletedCount} keys. Errors: ${errorCount}`);
            rl.close();
            return;
          }
