#!/usr/bin/env node
// scripts/generate-password-salt.js
// Generates a secure random string to use as a password salt
// and helps set it up in Cloudflare Workers.
...
async function main() {
  ...
  console.log('           Only use this for a fresh installation or if you plan to reset all passwords.');
  console.log('');
  ...
  try {
    ...
    if (shouldSaveToEnv.toLowerCase() === 'y') {
      ...
      if (fs.existsSync(envPath)) {
        ...
        // Remove existing PASSWORD_SALT line if it exists
    ...
    console.log('   - Generate admin password hash: node scripts/hash-password.js "your-secure-password"');
