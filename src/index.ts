...
app.use('*', async (c, next) => {
  ...
  if (host.includes('admin.farewellcafe.com') || c.req.path === '/admin') {
    ...
    // Simpler approach - serve admin page for /admin path, let client-side JS handle auth
    if (c.req.path === '/admin') {
      try {
        console.log(`[DEBUG] Serving admin dashboard for /admin path`);
        ...
        console.error('[ERROR] Failed to serve admin.html:', e);
      }
    }
    
    // Clear logic for session validation for other admin paths
    if (sessionToken) {
      // Try to validate JWT format
      try {
    ...
    console.log(`[DEBUG] No valid session, serving login page`);
